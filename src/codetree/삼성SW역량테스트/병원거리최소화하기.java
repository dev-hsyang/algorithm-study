...
/**
 * 병원을 선택하는 조합을 dfs 로 찾는 단계는 수월하게 했으나
 * 선택한 병원과 사람을 bfs 로 탐색하고 거리를 계산하려고 하여 메모리초과가 발생했다.
 * ArrayList 자료구조를 적절히 활용하여
 * (1) dfs 로 찾은 병원 조합 리스트
 * (2) 사람 리스트
 * (2) for 문을 통해 두 리스트를 돌며 사람별로 가장 가까운 병원을 찾으면 된다.
 */
